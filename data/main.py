...
def main(filename: str):
    ...
    # write train data
    pd.DataFrame(data=training_data, index=labels, columns=enc.columns).to_csv(file_path, index_label='label')

    # write meta data
    file_path_metadata = os.path.join(file_dir, f"train_data/{filename}.meta")
    with open(file_path_metadata, "w") as file:
        file.write(parser.metadata.__repr__())
